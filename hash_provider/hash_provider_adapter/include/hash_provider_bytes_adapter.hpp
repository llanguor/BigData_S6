...
private:
    ...
    std::shared_ptr<hash_provider_bytes<datatype>> _hash_provider;
...
public:
    ...
    {
        auto hash = _hash_provider->get_hash_code_from_raw(data, size);

        unsigned long long hash_combined = 0;

        for (int i = 0; i < _hash_provider->get_hash_size(); i+=8)
        {
